...
export function TaskDetails() {
  ...
  if (board && cmps.length === 0)
    setCmps(boardService.getTaskEditCmps(task, board))
...
function StatusPicker({ info, onUpdate }) {
  ...
  return (
    <section className="status-picker">
      <label htmlFor={id}>{info.label || "Status:"}</label>
      <select
        id={id}
        value={info.selectedStatus}
        onChange={(ev) => {
